...
const NativeSmartHomeScreen = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch data from backend for 'NativeSmartHome'
                const response = await homeAPI.getServiceDetails('NativeSmartHome');
                setFilters(response.data.filters);
                setServices(response.data.services);
            } catch (error) {
                console.error('Failed to fetch Native Smart Home data', error);

                // Fallback mock data if API fails momentarily (optional, but good for robust demo)
                // setFilters(['ALL', 'SMART LIGHTS', 'SENSORS']);
                // setServices([...]); 
            } finally {
                setLoading(false);
            }
                ...
                ListHeaderComponent={
                    <>
                        ...
                        <View style={styles.pageTitleContainer}>
                            ...
                            <View>
                                <Text style={styles.pageTitle}>NATIVE SMART</Text>
                                <Text style={[styles.pageTitle, { marginTop: 0 }]}>HOME HUB</Text>
                                <Text style={styles.resultsText}>{services.length} RESULTS FOUND</Text>
                            </View>
                        </View>

                        {/* Banner Image for context */}
                        <View style={styles.bannerContainer}>
                            <Image
                                source={{ uri: 'https://images.unsplash.com/photo-1558002038-109177381793?ixlib=rb-1.2.1&auto=format&fit=crop&w=1000&q=80' }}
                                style={styles.bannerBg}
                            />
                            <View style={styles.bannerOverlay}>
                                <Text style={styles.bannerText}>Upgrade to a smarter lifestyle.</Text>
                            </View>
                        </View>
...
const styles = StyleSheet.create({
    ...
    // Banner
    bannerContainer: { marginHorizontal: 20, borderRadius: 20, overflow: 'hidden', height: 150, marginBottom: 25 },
    bannerBg: { width: '100%', height: '100%' },
    bannerOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(0,0,0,0.4)', padding: 15 },
    ...
    cardContent: { flex: 1 },
    cardTitle: { fontSize: 16, fontWeight: 'bold', color: '#1A1025', marginBottom: 5, fontFamily: 'monospace' },
...
export default NativeSmartHomeScreen;
