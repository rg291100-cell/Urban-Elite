...
const NativeWaterPurifierScreen = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await homeAPI.getServiceDetails('NativeWaterPurifier');
                setFilters(response.data.filters);
                setServices(response.data.services);
            } catch (error) {
                console.error('Failed to fetch NativeWaterPurifier data', error);
            } finally {
                setLoading(false);
            }
        };
                ...
                ListHeaderComponent={
                    <>
                        ...
                        <View style={styles.pageTitleContainer}>
                            ...
                            <View>
                                <Text style={styles.pageTitle}>NATIVE WATER</Text>
                                <Text style={[styles.pageTitle, { marginTop: 0 }]}>PURIFIER</Text>
...
export default NativeWaterPurifierScreen;
