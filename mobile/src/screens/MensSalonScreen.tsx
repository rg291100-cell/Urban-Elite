...
const MensSalonScreen = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await homeAPI.getServiceDetails('MensSalon');
                setFilters(response.data.filters);
                setServices(response.data.services);
            } catch (error) {
                console.error('Failed to fetch MensSalon data', error);
            } finally {
                setLoading(false);
            }
        };
                ...
                ListHeaderComponent={
                    <>
                        ...
                        <View style={styles.pageTitleContainer}>
                            ...
                            <View>
                                <Text style={styles.pageTitle}>MEN'S SALON &</Text>
                                <Text style={[styles.pageTitle, { marginTop: 0 }]}>MASSAGE</Text>
...
export default MensSalonScreen;
