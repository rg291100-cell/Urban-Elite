...
const InstaHelpScreen = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await homeAPI.getServiceDetails('InstaHelp');
                setFilters(response.data.filters);
                setServices(response.data.services);
            } catch (error) {
                console.error('Failed to fetch InstaHelp data', error);
            } finally {
                setLoading(false);
            }
        };
                ...
                ListHeaderComponent={
                    <>
                        ...
                        <View style={styles.pageTitleContainer}>
                            ...
                            <View>
                                <Text style={styles.pageTitle}>INSTA HELP</Text>
...
const styles = StyleSheet.create({
    ...
    resultsText: { fontSize: 10, fontWeight: 'bold', color: '#A0AEC0', letterSpacing: 1, marginLeft: 5, marginTop: 2 },
...
export default InstaHelpScreen;
